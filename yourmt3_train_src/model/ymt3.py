...
class YourMT3(pl.LightningModule):
    ...
    def __init__(
            ...
            aux_onset_loss_weight: float = 0.0,
            aux_active_loss_weight: float = 0.0,
            aux_offset_loss_weight: float = 0.0,
            aux_focal_gamma: float = 0.0,
            aux_pos_weight_max: float = 64.0,
            aux_dropout_p: float = 0.1,
            aux_loss_only: bool = False,
            f0_adapter_enabled: bool = False,
            f0_adapter_bins: int = 88,
            f0_adapter_dropout: float = 0.1,
            f0_adapter_fmin: float = 70.0,
            f0_adapter_fmax: float = 1100.0,
            f0_adapter_ckpt: str = "./rmvpe_model.pt",
            f0_adapter_gate_init: float = 0.0,
            f0_adapter_channel: int = -1,
            **kwargs: Any) -> None:
        super().__init__()
        ...
        self.aux_frame_proj = nn.Sequential(
            nn.LayerNorm(decoder_config["d_model"]),
            nn.Linear(decoder_config["d_model"], decoder_config["d_model"]),
            nn.GELU(),
            nn.Dropout(aux_dropout_p),
        )
        self.aux_active_head = nn.Linear(decoder_config["d_model"], 1)
        self.aux_onset_head = nn.Linear(decoder_config["d_model"], 1)
        self.aux_offset_head = nn.Linear(decoder_config["d_model"], 1)
        self.f0_adapter_enabled = bool(f0_adapter_enabled)
        self.f0_adapter_bins = int(f0_adapter_bins)
        self.f0_adapter_fmin = float(f0_adapter_fmin)
        self.f0_adapter_fmax = float(f0_adapter_fmax)
        self.f0_adapter_ckpt = str(f0_adapter_ckpt)
        self.f0_adapter_channel = int(f0_adapter_channel)
        self._rmvpe = None
        if self.f0_adapter_enabled:
            f0_in_dim = self.f0_adapter_bins + 2  # coarse pitch bins + voiced + normalized pitch
            self.f0_adapter = nn.Sequential(
                nn.LayerNorm(f0_in_dim),
                nn.Linear(f0_in_dim, decoder_config["d_model"]),
                nn.GELU(),
                nn.Dropout(f0_adapter_dropout),
                nn.Linear(decoder_config["d_model"], decoder_config["d_model"]),
            )
            self.f0_adapter_gate = nn.Parameter(torch.tensor(float(f0_adapter_gate_init)))
        else:
            self.f0_adapter = None
            self.register_parameter("f0_adapter_gate", None)
    ...
    def _balanced_frame_bce(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        targets = targets.to(device=logits.device, dtype=logits.dtype)
        pos = targets.sum()
        pos_weight_max = float(getattr(self.hparams, "aux_pos_weight_max", 64.0))
        if pos.item() <= 0:
            bce = F.binary_cross_entropy_with_logits(logits, targets, reduction='none')
        else:
            neg = targets.numel() - pos
            pos_weight = torch.clamp(
                neg / pos, min=1.0, max=pos_weight_max).to(device=logits.device, dtype=logits.dtype)
            bce = F.binary_cross_entropy_with_logits(logits, targets, pos_weight=pos_weight, reduction='none')
        gamma = float(getattr(self.hparams, "aux_focal_gamma", 0.0))
        if gamma > 0:
            prob = torch.sigmoid(logits)
            pt = torch.where(targets > 0.5, prob, 1.0 - prob)
            bce = bce * torch.pow((1.0 - pt).clamp(min=1e-4), gamma)
        return bce.mean()

    def _compute_frame_aux_loss(
            self,
            enc_hs: torch.Tensor,
            frame_targets: Optional[torch.FloatTensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        zero = enc_hs.new_zeros(())
        onset_w = float(self.hparams.aux_onset_loss_weight)
        active_w = float(self.hparams.aux_active_loss_weight)
        offset_w = float(getattr(self.hparams, "aux_offset_loss_weight", 0.0))
        if frame_targets is None or (onset_w <= 0 and active_w <= 0 and offset_w <= 0):
            return zero, zero, zero, zero

        logits = self.frame_aux_logits_from_hidden(enc_hs, target_len=frame_targets.shape[-1])
        if logits is None:
            return zero, zero, zero, zero

        active_logits, onset_logits, offset_logits = logits
        frame_targets = frame_targets.to(device=active_logits.device, dtype=active_logits.dtype)
        active_targets = frame_targets[:, 0]
        onset_targets = frame_targets[:, 1]
        offset_targets = frame_targets[:, 2] if frame_targets.shape[1] > 2 else None
        aux_active_loss = self._balanced_frame_bce(active_logits, active_targets) if active_w > 0 else zero
        aux_onset_loss = self._balanced_frame_bce(onset_logits, onset_targets) if onset_w > 0 else zero
        aux_offset_loss = (
            self._balanced_frame_bce(offset_logits, offset_targets)
            if offset_w > 0 and offset_targets is not None else zero
        )
        aux_loss = aux_active_loss * active_w + aux_onset_loss * onset_w + aux_offset_loss * offset_w
        return aux_loss, aux_active_loss, aux_onset_loss, aux_offset_loss

    def frame_aux_logits_from_hidden(
            self,
            enc_hs: torch.Tensor,
            target_len: Optional[int] = None) -> Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        # Multi-channel YourMT3+ produces enc_hs of shape (B, K, T, D); collapse channel dim
        # by simple mean so aux supervision and inference use the same representation.
        if enc_hs.dim() == 4:
            enc_aux = enc_hs.mean(dim=1)
        elif enc_hs.dim() == 3:
            enc_aux = enc_hs
        else:
            return None

        aux_hs = self.aux_frame_proj(enc_aux)
        active_logits = self.aux_active_head(aux_hs).squeeze(-1)
        onset_logits = self.aux_onset_head(aux_hs).squeeze(-1)
        offset_logits = self.aux_offset_head(aux_hs).squeeze(-1)
        if target_len is not None and active_logits.shape[-1] != target_len:
            stacked_logits = torch.stack([active_logits, onset_logits, offset_logits], dim=1)
            stacked_logits = F.interpolate(stacked_logits, size=target_len, mode='linear', align_corners=False)
            active_logits = stacked_logits[:, 0]
            onset_logits = stacked_logits[:, 1]
            offset_logits = stacked_logits[:, 2]
        return active_logits, onset_logits, offset_logits

    def encode_audio_for_decoder(
            self,
            x: torch.FloatTensor,
            task_tokens: Optional[torch.LongTensor] = None) -> torch.Tensor:
        raw_audio = x
        ...
        return self._apply_f0_adapter(enc_hs, raw_audio)

    def frame_aux_logits(
            self,
            x: torch.FloatTensor,
            task_tokens: Optional[torch.LongTensor] = None,
            target_len: Optional[int] = None) -> Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        enc_hs = self.encode_audio_for_decoder(x, task_tokens=task_tokens)
        return self.frame_aux_logits_from_hidden(enc_hs, target_len=target_len)

    def _load_rmvpe(self):
        if self._rmvpe is not None:
            return self._rmvpe
        os.environ.setdefault("NUMBA_DISABLE_JIT", "1")
        rosvot_root = os.path.dirname(os.path.dirname(os.path.dirname(self.f0_adapter_ckpt)))
        saved_modules = {
        ...
        sys.path.insert(0, rosvot_root)
        try:
            from modules.pe.rmvpe import RMVPE
            self._rmvpe = RMVPE(self.f0_adapter_ckpt, device=self.device)
        finally:
            try:
                sys.path.remove(rosvot_root)
            except ValueError:
                pass
            for name in [n for n in list(sys.modules.keys()) if n == "utils" or n.startswith("utils.")]:
                if name not in saved_modules:
                    sys.modules.pop(name, None)
            sys.modules.update(saved_modules)
        return self._rmvpe

    def _f0_features_from_audio(self, raw_audio: torch.Tensor, target_len: int, dtype: torch.dtype) -> torch.Tensor:
        rmvpe = self._load_rmvpe()
        audio = raw_audio.detach()
        if audio.dim() == 3:
        ...
        with torch.no_grad():
            f0_list = [
                rmvpe.infer_from_audio(wav, sample_rate=int(self.audio_cfg["sample_rate"]))
                for wav in audio_np
            ]

        feats = []
        for f0 in f0_list:
            f0 = rmvpe.postprocess(f0.copy(), fmin=self.f0_adapter_fmin, fmax=self.f0_adapter_fmax, min_gap=6)
            f0_t = torch.as_tensor(f0, device=self.device, dtype=torch.float32).view(1, 1, -1)
            if f0_t.shape[-1] != target_len:
                f0_t = F.interpolate(f0_t, size=target_len, mode="linear", align_corners=False)
            f0_t = f0_t.view(target_len)
            voiced = (f0_t > 0).to(torch.float32)
            midi = torch.zeros_like(f0_t)
            midi = torch.where(f0_t > 0, 69.0 + 12.0 * torch.log2(torch.clamp(f0_t, min=1.0) / 440.0), midi)
            coarse = torch.clamp(torch.round(midi) - 21, 0, self.f0_adapter_bins - 1).to(torch.long)
            one_hot = F.one_hot(coarse, num_classes=self.f0_adapter_bins).to(torch.float32) * voiced.unsqueeze(-1)
            norm_pitch = torch.clamp((midi - 60.0) / 24.0, -2.0, 2.0) * voiced
            feats.append(torch.cat([one_hot, voiced.unsqueeze(-1), norm_pitch.unsqueeze(-1)], dim=-1))
        return torch.stack(feats, dim=0).to(dtype=dtype)

    def _apply_f0_adapter(self, enc_hs: torch.Tensor, raw_audio: torch.Tensor) -> torch.Tensor:
        if not self.f0_adapter_enabled or self.f0_adapter is None:
            return enc_hs
        if enc_hs.dim() == 4:
            target_len = enc_hs.shape[2]
        elif enc_hs.dim() == 3:
            target_len = enc_hs.shape[1]
        else:
            return enc_hs
        f0_feats = self._f0_features_from_audio(raw_audio, target_len, enc_hs.dtype)
        f0_emb = self.f0_adapter(f0_feats)
        gate = torch.tanh(self.f0_adapter_gate).to(dtype=enc_hs.dtype)
        if enc_hs.dim() == 4:
            f0_add = gate * f0_emb
            channel = int(getattr(self, "f0_adapter_channel", -1))
            if 0 <= channel < enc_hs.shape[1]:
                out = enc_hs.clone()
                out[:, channel, :, :] = out[:, channel, :, :] + f0_add
                return out
            return enc_hs + f0_add.unsqueeze(1)
        return enc_hs + gate * f0_emb

    def forward(
            self,
            x: torch.FloatTensor,
            target_tokens: torch.LongTensor,
            frame_targets: Optional[torch.FloatTensor] = None,
            # task_tokens: Optional[torch.LongTensor] = None,
            **kwargs) -> Dict:
        ...
        enc_hs = self._apply_f0_adapter(enc_hs, raw_audio)
        aux_loss, aux_active_loss, aux_onset_loss, aux_offset_loss = self._compute_frame_aux_loss(enc_hs, frame_targets)
        if bool(getattr(self.hparams, "aux_loss_only", False)):
            ...
            return {
                ...
                "token_loss": zero,
                "aux_loss": aux_loss,
                "aux_active_loss": aux_active_loss,
                "aux_onset_loss": aux_onset_loss,
                "aux_offset_loss": aux_offset_loss,
            }
    ...
    def training_step(self, batch, batch_idx) -> torch.Tensor:
        ...
        # 'dataset3': ...
        # }
        batch_items = [torch.cat(t, dim=0) for t in zip(*batch.values())]
        if len(batch_items) == 4:
            audio_segments, note_tokens, pshift_steps, frame_targets = batch_items
        else:
            audio_segments, note_tokens, pshift_steps = batch_items
            frame_targets = None
        ...
        outputs = self(audio_segments, note_tokens, frame_targets=frame_targets)
        loss = outputs['loss']
        self.log('train_loss',
                 loss,
                 on_step=True,
        ...
        if frame_targets is not None:
            self.log('train_aux_loss',
                     ...
                     batch_size=note_tokens.shape[0],
                     sync_dist=True)
            self.log('train_aux_active_loss',
                     outputs['aux_active_loss'],
                     on_step=True,
                     on_epoch=True,
                     prog_bar=False,
                     batch_size=note_tokens.shape[0],
                     sync_dist=True)
            self.log('train_aux_onset_loss',
                     outputs['aux_onset_loss'],
                     on_step=True,
                     on_epoch=True,
                     prog_bar=False,
                     batch_size=note_tokens.shape[0],
                     sync_dist=True)
            self.log('train_aux_offset_loss',
                     outputs['aux_offset_loss'],
                     on_step=True,
                     on_epoch=True,
                     prog_bar=False,
                     batch_size=note_tokens.shape[0],
